...
export default function DynamicLanesViz(props) {
    ...
    const computeExcerpts = (data) => {
        ...
        data.sequence.forEach((id) => {
            ...
            if (showExcerpts && !paper.hidden) {
                ...
                if (focus.CitCon?.[paper.Id]) {
                    paper.excerpts = focus.CitCon[paper.Id].map(truncateExcerpt);
    ...
    // Rendering
    // Draw paper titles
    const makeTitle = (data, p) => p.Ti.split(" ").reduce((acc, w, i) => {
        const keyword = stemmer(w);
        if (!acc.tagged.includes(keyword) &&
            data.keywords[keyword] &&
            data.edges[keyword].some(e => e.some(point => String(point.pid) === String(p.Id)))) {
            if (selectedKW.includes(keyword)) {
                acc.result.push(<tspan key={p.Id + `[${i}]`} className={'keyword'}
                ...
                acc.result.push(<>
                    <tspan key={p.Id + `[${i}]`}
                           className={(data.edges[keyword].some(e => e.length > params.minKeywordHighlight && e.some(point => String(point.pid) === String(p.Id)))) ? 'keyword unselected' : ''}
                           onContextMenu={makeMenuHandler(p, keyword)}
                           onClick={(event) => {
        ...
        } else acc.result.push(<tspan key={p.Id + `[${i}]`}>{w + " "}</tspan>);
        return acc;
                        ...
                        <g key={`${keyword}_${i}`} className={'kwhighlights'}>
                            {edges.map((edge) => edge.filter(({pid}) => !data.papers[pid].hidden).map(({x, pid}) =>
                                <rect
                                    ...
                                    y={data.papers[pid].layout.y - params.charHeight / 2}
                                    width={((
                                        data.papers[pid].Ti.indexOf(" ",
                                            data.papers[pid].kwOffsets[keyword]) + 1 || data.papers[pid].Ti.length + 1)
                        ...
                        <g key={p.Id}>
                            <g onClick={(event) => {
                                ...
                                <text
                                    ...
                                    ({p.Y}) {makeTitle(data, p)}
                                    <tspan className={'citecount'}> {String(p.Id) === String(paperID) ?
                                        <tspan style={{
                                            fill: '#f00',
                                            fontWeight: 'bold'
                                        }}>FOCAL</tspan>
                                        :
                                        (expansions.includes(String(p.Id)) && <tspan style={{
                                            fill: '#080',
                                            fontWeight: 'bold'
                                        }}>EXPANDED</tspan>)
                                    } &lt;{p.CC}&gt; {p.VSN} {Array(Math.ceil(Math.log2(1 + p.CC / data.maxCitations * 31))).fill("*").join("")}
                                    </tspan>
                                </text>
                            </g>
                            {showExcerpts && p.excerpts && <g>
                                {p.excerpts.map((excerpt, i) =>
                                    <text className={'viz-excerpt'}
                                          ...
                                          onClick={p.excerptsType === 'citedByFocus' ? makeHighlightExcerpt(data.focus, p.Id, i) : makeHighlightExcerpt(p.Id, data.focus, i)}>
